...
func getSimulateFromSeedInput(tb testing.TB, w io.Writer, app *DropApp) (
...
func testAndRunTxs(app *DropApp) []simulation.WeightedOperation {
	cdc := MakeCodec()
...
func invariants(app *DropApp) []sdk.Invariant {
...
// Profile with:
// /usr/local/go/bin/go test -benchmem -run=^$ github.com/cosmos/cosmos-sdk/DropApp -bench ^BenchmarkFullAppSimulation$ -Commit=true -cpuprofile cpu.out
func BenchmarkFullAppSimulation(b *testing.B) {
	...
	app := NewDropApp(logger, db, nil, true, 0)
...
func TestFullAppSimulation(t *testing.T) {
	...
	app := NewDropApp(logger, db, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "DropApp", app.Name())
...
func TestAppImportExport(t *testing.T) {
	...
	newApp := NewDropApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
...
func TestAppSimulationAfterImport(t *testing.T) {
	...
	newApp := NewDropApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "DropApp", newApp.Name())
